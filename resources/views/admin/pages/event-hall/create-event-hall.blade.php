...
<x-layout.dashboard-panel>
   <!-- Entête de la page -->
   <x-admin.events.create.event-hall-header :agence="$agence" />
   ...
   <form action="{{ route('admin.event-hall.store', $agence->id) }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 gap-8 lg:grid-cols-3">
      ...
      <div class="lg:col-span-1">
          <x-admin.events.create.event-hall-images :errors="$errors" />
      </div>
      
      <!-- Form Fields -->
      <div class="lg:col-span-2">
          <x-admin.events.create.event-hall-form :agence="$agence" :villes="$villes" />
      </div>
   </form>
</x-layout.dashboard-panel>
@endsection
