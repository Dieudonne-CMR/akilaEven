...
<x-layout.dashboard-panel class="">
    ...
    <div class="">
      <div class="max-w-full p-6 mb-6 bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700">
          <!-- Bouton Retour -->
          <a href="{{ route("admin.agences") }}" class="inline-flex items-center mb-6 text-gray-600 transition-colors hover:text-blue-600">
          ...
          <!-- En-tête -->
          <header class="mb-8">
              <h1 class="mb-2 text-3xl font-bold text-gray-900">Ajouter un nouvel agence</h1>
      ...
      <div class="p-6 mb-8 bg-white rounded-lg shadow-md">
          <div class="mb-8">
              <h2 class="mb-1 text-xl font-semibold text-gray-900">Informations sur l'agence</h2>
          ...
          <form action="{{ route('admin.agences.store') }}" method="POST" enctype="multipart/form-data">
              ...
              <!-- Nom de l'agence -->
              <div class="mb-6">
                  <label for="nom_agence" class="block mb-2 text-sm font-medium text-gray-900">
                      Nom de l'agence <span class="text-red-500">*</span>
                  </label>
                  <input type="text" id="nom_agence" name="nom_agence" value="{{ old('nom_agence')}}" class="bg-gray-50 border {{ session()->has('errors') && session('errors')->has('nom_agence') ? 'border-red-500' : 'border-gray-300'}} text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="Entrez le nom de l'agence" required>
                  @if(session()->has('errors') && session('errors')->has('nom_agence'))
                      <p class="mt-1 text-sm text-red-600">{{ session('errors')->first('nom_agence') }}</p>
                  @endif
              </div>
              ...
              <div class="grid grid-cols-1 gap-6 mb-6 md:grid-cols-2">
                  <div>
                      ...
                      <div class="relative">
                          ...
                          <input type="email" id="email" name="email" value="{{ old('email') }}" class="bg-gray-50 border {{ session()->has('errors') && session('errors')->has('email') ? 'border-red-500' : 'border-gray-300' }} text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5" placeholder="agence@exemple.com" required>
                      </div>
                  ...
                  <div class="relative">
                      ...
                      <select id="ville" name="ville" class="bg-gray-50 border {{ session()->has('errors') && session('errors')->has('ville') ? 'border-red-500' : 'border-gray-300' }} text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5" required>
                          ...
                          @foreach($villes as $ville)
                              <option value="{{ $ville->nom }}" {{ old('ville') == $ville->nom ? 'selected' : '' }}>{{ $ville->nom }}</option>
                          @endforeach
                      </select>
                  </div>
              ...
              <!-- Description -->
              <div class="mb-6">
                  <label for="description_agence" class="block mb-2 text-sm font-medium text-gray-900">
                      Description <span class="text-red-500">*</span>
                  </label>
                  <textarea id="description_agence" name="description_agence" rows="4" class="bg-gray-50 border {{ session()->has('errors') && session('errors')->has('description_agence') ? 'border-red-500' : 'border-gray-300' }} text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="Décrivez votre agence, ses caractéristiques uniques et ce que les clients peuvent attendre..." required>{{ old('description_agence') }}</textarea>
                  @if(session()->has('errors') && session('errors')->has('description_agence'))
                      <p class="mt-1 text-sm text-red-600">{{ session('errors')->first('description_agence') }}</p>
                  @endif
              </div>
              ...
              <div class="mb-6">
                  ...
                  <div class="relative">
                      ...
                      <input type="text" id="localisation" name="localisation" value="{{ old('localisation') }}" class="bg-gray-50 border {{ session()->has('errors') && session('errors')->has('localisation') ? 'border-red-500' : 'border-gray-300' }} text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5" placeholder="Entrez l'adresse complète de l'agence" required>
                  </div>
              ...
              <div class="mb-6" x-data="tagInput()" x-init="init()">
                  ...
                  <p class="mt-1 text-sm text-gray-500">Ajoutez jusqu'à 10 services ou équipements proposés par votre agence</p>
              ...
              <div class="mb-8">
                  <label class="block mb-2 text-sm font-medium text-gray-900">
                      Bannières de l'agence <span class="text-red-500">*</span>
                  </label>
                  <p class="mb-3 text-sm text-gray-500">Uploader au moins une image de haute qualité montrant votre agence</p>
