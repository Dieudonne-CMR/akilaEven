...
<x-layout.dashboard-panel>
   <!-- Entête de la page -->
   <x-admin.locations.create.location-header :agence="$agence" />
  
   <!-- Formulaire -->
   <form action="{{ route('admin.location.store', $agence->id) }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 gap-8 lg:grid-cols-3">
       @csrf
      <!-- Sidebar - Images Upload -->
      <div class="lg:col-span-1">
          <x-admin.locations.create.location-images :errors="$errors" />
      </div>
      
      <!-- Form Fields -->
      <div class="lg:col-span-2">
          <x-admin.locations.create.location-form :agence="$agence" :villes="$villes" />
      </div>

      <!-- Submit Button -->
      <div class="lg:col-span-3">
          <div class="flex justify-end">
              <button type="submit" class="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-blue-600 border border-transparent rounded-lg shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                  Créer la location
              </button>
          </div>
      </div>
        ...
        const editor = new Editor({
            ...
            extensions: [
                ...
                Placeholder.configure({
                    placeholder: 'Règlement de la location...',
