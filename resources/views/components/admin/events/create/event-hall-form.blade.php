...
<div class="p-6 bg-white rounded-lg shadow-sm">
    ...
    <div class="space-y-6">
        ...
        <div>
            ...
            <div class="relative">                          
                <input 
                    ...
                    name="nom_salle" 
                    value="{{ old('nom_salle') }}"
                    class="bg-gray-50 border {{ session()->has('errors') && session('errors')->has('nom_salle') ? 'border-red-500' : 'border-gray-300' }} text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                    required
                >
            </div>
            @if(session()->has('errors') && session('errors')->has('nom_salle'))
                <p class="mt-1 text-sm text-red-600">{{ session('errors')->first('nom_salle') }}</p>
            @endif
        </div>
        ...
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
            ...
            <!-- Localisation -->
            <div>
                <label for="localisation" class="block mb-2 text-sm font-medium text-gray-700">Adresse/Emplacement précis <span class="text-red-500">*</span></label>
                <div class="relative">
                    <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                    ...
                    <input 
                        ...
                        class="bg-gray-50 border {{ session()->has('errors') && session('errors')->has('localisation') ? 'border-red-500' : 'border-gray-300' }} text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5" 
        ...
        <div class="grid grid-cols-1 gap-6 md:grid-cols-3">
            <!-- Capacity -->
            <div>
                <label for="capacite" class="block mb-2 text-sm font-medium text-gray-700">Capacité <span class="text-red-500">*</span></label>
                <div class="relative">
                    <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                    ...
                    <input 
                        ...
                        name="capacite"
                        value="{{ old('capacite') }}"
                        class="bg-gray-50 border {{ session()->has('errors') && session('errors')->has('capacite') ? 'border-red-500' : 'border-gray-300' }} text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5" 
                        placeholder="ex: 150"                                  
                        required
                    >
                </div>
                @if(session()->has('errors') && session('errors')->has('capacite'))
                    <p class="mt-1 text-sm text-red-600">{{ session('errors')->first('capacite') }}</p>
                @endif
            </div>
            ...
            <!-- Price -->
            <div>
                <label for="prix" class="block mb-2 text-sm font-medium text-gray-700">Prix <span class="text-red-500">*</span></label>
                <div class="relative">     
                    <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                    ...
                    <input
                        ...
                        name="prix"
                        value="{{ old('prix') }}" 
                        class="bg-gray-50 border {{ session()->has('errors') && session('errors')->has('prix') ? 'border-red-500' : 'border-gray-300' }} text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 pr-24 p-2.5"
                ...
                @if(session()->has('errors') && session('errors')->has('prix'))
                    <p class="mt-1 text-sm text-red-600">{{ session('errors')->first('prix') }}</p>
                @endif
            </div>
        </div>
        ...
        <!-- Description -->
        <div>
            <label for="description_salle" class="block mb-2 text-sm font-medium text-gray-700">Description <span class="text-red-500">*</span></label>
            <textarea 
                id="description_salle" 
                name="description_salle" 
                rows="4" 
                class="bg-gray-50 border {{ session()->has('errors') && session('errors')->has('description_salle') ? 'border-red-500' : 'border-gray-300' }} text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" 
                placeholder="Décrivez la salle en détail..."
                required
            >{{ old('description_salle') }}</textarea>
            @if(session()->has('errors') && session('errors')->has('description_salle'))
                <p class="mt-1 text-sm text-red-600">{{ session('errors')->first('description_salle') }}</p>
            @endif
        </div>
