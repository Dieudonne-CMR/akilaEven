...
<div class="p-6 bg-white rounded-lg shadow-sm lg:sticky lg:top-8">
    ...
    <div class="mb-6">
        ...
        <div 
            class="relative p-6 text-center transition-colors border-2 rounded-lg cursor-pointer hover:bg-gray-50 {{ session()->has('errors') && session('errors')->has('main_image') ? 'border-red-500' : 'border-gray-300 border-dashed' }}"
        ...
        @if(session()->has('errors') && session('errors')->has('main_image'))
            <p class="mt-1 text-sm text-red-600">{{ session('errors')->first('main_image') }}</p>
        @endif
    </div>
