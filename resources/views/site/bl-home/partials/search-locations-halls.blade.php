...
<div class="w-full max-w-6xl" x-data="searchForm()">
  ...
  <div x-show="activeTab === 'locations'" class="p-2 bg-white rounded-lg shadow-lg sm:p-3">
    <form @submit.prevent="submitSearch('locations')">
      <div class="flex flex-col items-center gap-3 sm:flex-row">
        <div class="w-full sm:w-1/5">
          <label for="location-locations" class="sr-only">Lieu</label>
          <div class="relative">
            <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
              <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-gray-400" viewBox="0 0 20 20" fill="currentColor">
                <path fill-rule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clip-rule="evenodd" />
            ...
            <input 
              ...
              x-model="locations.location" 
              list="locations-locations" 
              name="location" 
              placeholder="Destination" 
              class="block w-full py-3 pl-10 pr-3 leading-5 placeholder-gray-500 bg-gray-100 border border-transparent rounded-md focus:outline-none focus:ring-2 focus:ring-purple-600 focus:bg-white">
            <datalist id="locations-locations">
              <template x-for="location in availableLocations" :key="location">
                <option :value="location"></option>
              </template>
            </datalist>
        ...
        <div class="w-full sm:w-1/5">
          <label for="checkin-locations" class="sr-only">Arrivée</label>
          <div class="relative">
            <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
              <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-gray-400" viewBox="0 0 20 20" fill="currentColor">
                <path fill-rule="evenodd" d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z" clip-rule="evenodd" />
        ...
        <div class="w-full sm:w-1/5">
          <label for="checkout-locations" class="sr-only">Départ</label>
          <div class="relative">
            <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
              <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-gray-400" viewBox="0 0 20 20" fill="currentColor">
                <path fill-rule="evenodd" d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z" clip-rule="evenodd" />
            ...
            <input 
              id="checkout-locations" 
              x-model="locations.checkOut" 
              name="checkout" 
              type="date" 
              :min="locations.checkIn || '<?= date('Y-m-d') ?>'"
              placeholder="Départ" 
              class="block w-full py-3 pl-10 pr-3 leading-5 placeholder-gray-500 bg-gray-100 border border-transparent rounded-md focus:outline-none focus:ring-2 focus:ring-purple-600 focus:bg-white">
          </div>
        </div>
        ...
        <div class="w-full sm:w-1/5">
          <label for="guests-locations" class="sr-only">Voyageurs</label>
          <div class="relative">
            <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
              <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-gray-400" viewBox="0 0 20 20" fill="currentColor">
                <path d="M9 6a3 3 0 11-6 0 3 3 0 016 0zM17 6a3 3 0 11-6 0 3 3 0 016 0zM12.93 17c.046-.327.07-.66.07-1a6.97 6.97 0 00-1.5-4.33A5 5 0 0119 16v1h-6.07zM6 11a5 5 0 015 5v1H1v-1a5 5 0 015-5z" />
    ...
    Alpine.data('searchForm', () => ({
      ...
      submitSearch(type) {
        ...
        if (type === 'venues') {
          ...
          if (this.locations.location) searchParams.append('location', this.locations.location);
          if (this.locations.checkIn) searchParams.append('checkin', this.locations.checkIn);
          if (this.locations.checkOut) searchParams.append('checkout', this.locations.checkOut);
          if (this.locations.guests) searchParams.append('guests', this.locations.guests);
        }
